...
int main(int argc, char *argv[])
{
	...
	if (argc == 7)
	{
		grad = atoi(argv[3]);
		if (grad != 0 && grad != 1)
			err("Gradient != 1 and gradient != 0");
		dithering = atoi(argv[4]);
		if (dithering < 0 || dithering > 7)
			err("Dithering should be in [0..7]");
		bitness = atoi(argv[5]);
		if (bitness < 1 || bitness > 8)
			err("Bit should be in [1..8]");
		gamma_ = atof(argv[6]);
		if (gamma_ < 0)
			err("Gamma should be >= 0");
	}
	else
